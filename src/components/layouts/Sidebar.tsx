...
const MenuItem = memo(({ href, iconName, suffix, children: label, onClick, visible = true }) => {
    if (!visible) {
        return null;
    }
    const Component = href ? Link : Button;
    return (<List.Item>
        <Component width="100%" variant="unstyled" padding={2} href={href} iconName={iconName} suffix={suffix} onClick={onClick} backgroundColor={{
        default: 'none',
        hover: getValueForMenuItem('hover.backgroundColor'),
    }} spacing={{ default: 3, s: 0 }} stackOn={Breakpoints.S} customChildren title={label}>
          <Text flex={1} css={`
              white-space: nowrap;
              overflow: hidden;
              text-overflow: ellipsis;
            `} fontSize={{ s: '9px' }} textAlign={{ default: 'left', s: 'center' }}>
            {label}
          </Text>
        </Component>
      </List.Item>);
});
(MenuItem as any).propTypes = {
...
const Menu = memo(({ items = [], title, ...props }) => {
    const Content = (contentProps) => (<List {...contentProps}>
      {items.map((menuItem, index) => (<MenuItem key={index} {...menuItem}/>))}
    </List>);
    if (!title)
        return <Content {...props}/>;
    return (<Stack spacing={3} {...props}>
      <Title opacity={0.5} css={`
          font-size: 13px;
          font-weight: 400;
...
const ProfileMenu = ({ profileImage }) => {
    ...
    return (<Inline padding={1} spacing={2} alignItems="center" justifyContent="center" css={`
...
const NotificationMenu = memo(({ countUnseenAnnouncements, onClickAnnouncementsButton, onClickJobLoggerButton, jobLoggerState, }) => {
    const { t, i18n } = useTranslation();
    const notificationMenu = [
        {
            iconName: Icons.GIFT,
            children: t('menu.announcements'),
            suffix: countUnseenAnnouncements > 0 && (<Badge>{countUnseenAnnouncements}</Badge>),
...
(NotificationMenu as any).propTypes = {
...
const Sidebar = () => {
    const { t } = useTranslation();
    const [isReactCreateFeatureFlagEnabled] = useFeatureFlag(FeatureFlags.REACT_CREATE);
    ...
    const handleClickAnnouncement = useCallback((activeAnnouncement) => setActiveAnnouncementId(activeAnnouncement.uid), []);
    const toggleIsAnnouncementsModalVisible = useCallback(() => setIsAnnouncementsModalVisible((prevValue) => !prevValue), []);
    const toggleIsJobLoggerVisible = useCallback(() => setIsJobLoggerVisible((prevValue) => !prevValue), []);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const seenAnnouncements = cookies?.seenAnnouncements ?? [];
        const cleanedUpSeenAnnouncements = seenAnnouncements.filter((seenAnnouncementId) => rawAnnouncements.find((announcement) => announcement.uid === seenAnnouncementId));
    ...
    useEffect(() => {
        ...
        const validationQuery = getRolesQuery.data
            .map((role) => role.constraints !== undefined && role.constraints.v3
            ? role.constraints.v3
            : null)
    ...
    const announcements = useMemo(() => rawAnnouncements.map((announcement) => {
    ...
    const countUnseenAnnouncements = useMemo(() => announcements.filter((announcement) => announcement.status === AnnouncementStatus.UNSEEN).length, [announcements]);
    const userMenu = [
        {
            href: '/dashboard',
            iconName: Icons.HOME,
    ...
    const manageMenu = [
        {
            ...
            suffix: countEventsToModerate > 0 && (<Badge>{countEventsToModerate}</Badge>),
        },
        {
    ...
    const filteredManageMenu = useMemo(() => {
        ...
        return manageMenu.filter((menuItem) => getPermissionsQuery.data.includes(menuItem.permission));
    }, [getPermissionsQuery.data]);
    return [
        <Stack key="sidebar" forwardedAs="nav" height="100%" css={`
        overflow: hidden;
      `} width={{ default: '230px', s: '65px' }} backgroundColor={getValueForSidebar('backgroundColor')} color={getValueForSidebar('color')} zIndex={1998} padding={{ default: 2, s: 0 }} spacing={3} ref={sidebarComponent} onMouseOver={() => {
            setTimeout(() => {
                if (!sidebarComponent?.current)
                    return;
                sidebarComponent.current.focus();
            }, 100);
        }}>
      <Link justifyContent="center" href="/dashboard" title={t('menu.home')} customChildren>
        <Logo variant={isSmallView ? LogoVariants.MOBILE : LogoVariants.DEFAULT}/>
      </Link>
      <Stack paddingTop={4} spacing={4} flex={1} css={`
          > :not(:first-child) {
            border-top: 1px solid ${getValueForMenu('borderColor')};
          }
        `}>
        <Menu items={userMenu}/>
        <Stack justifyContent={filteredManageMenu.length > 0 ? 'space-between' : 'flex-end'} flex={1}>
          {filteredManageMenu.length > 0 && (<Menu items={filteredManageMenu} title={t('menu.management')}/>)}
          <Stack>
            <NotificationMenu countUnseenAnnouncements={countUnseenAnnouncements} jobLoggerState={jobLoggerState} onClickAnnouncementsButton={toggleIsAnnouncementsModalVisible} onClickJobLoggerButton={toggleIsJobLoggerVisible}/>
            <ProfileMenu />
          </Stack>
        </Stack>
      </Stack>
    </Stack>,
        <JobLogger key="joblogger" visible={isJobLoggerVisible} onClose={() => setIsJobLoggerVisible((oldState) => !oldState)} onStatusChange={setJobLoggerState}/>,
        <Announcements key="announcements" visible={isAnnouncementsModalVisible} announcements={announcements || []} onClickAnnouncement={handleClickAnnouncement} onClose={toggleIsAnnouncementsModalVisible}/>,
    ];
};
export { Sidebar };
