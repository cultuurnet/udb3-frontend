...
describe('fetchFromApi', () => {
    ...
    it('throws error on fetch error', async () => {
        ...
        (fetch as any).mockReject(new Error(message));
        await expect(fetchFromApi({
            path: '/random',
    ...
    it('throws error on not ok response', async () => {
        const message = 'This is an error';
        (fetch as any).mockResponseOnce(JSON.stringify({ title: message }), { status: 400 });
        await expect(fetchFromApi({
            path: '/random',
        })).rejects.toThrowError(message);
    });
    it('returns error on not ok response (silentError mode)', async () => {
        const message = 'This is an error';
        (fetch as any).mockResponseOnce(JSON.stringify({ title: message }), { status: 400 });
        await expect(fetchFromApi({ path: '/random', silentError: true })).resolves.toStrictEqual({
            message: message,
            status: 400,
            type: 'ERROR',
        });
