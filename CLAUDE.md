...
`scripts/vrt/env.mjs` holds what a run assumes about its environment — the feature flags, the values it pins so a baseline renders the same locally and in CI, the required-variable check, the list of hosts a run is allowed to reach, and the `dotenv` load. Open it to change any of that. `scripts/vrt/hosts.mjs` holds the addresses the app, the mock server and the browser use to reach each other; fixture modules may import it, but nothing Playwright loads may, because inside the container it resolves to the container's own `localhost`.

Page-level VRT runs the real app with API calls intercepted by a local mock server (`scripts/vrt/mock-server.mjs`). Anything not explicitly mocked falls through to the real backend and logs that it did — coverage builds incrementally: screenshot a page first, see what looks live, mock exactly that. That report can only see hosts registered in `MOCK_UPSTREAMS`, so a run also fails if the browser reaches any host outside the allowlist in `env.mjs` — the fixtures' own image host included.
...
1. Add an entry to the `screenshotPages([...])` call in the relevant `src/test/vrt-pages/<domain>.spec.ts` file (create the file if the domain doesn't have one yet, calling `screenshotPages` from `src/test/vrt-pages/support.ts`) with a `title` and `path` — this declares the test, navigates, and screenshots in one go. The baseline filename is derived automatically from the title and the spec file as `pages--<title with spaces as dashes>--<spec file>.png` (e.g. `pages--labels-overview--labels.spec.ts.png`), so name entries accordingly. `screenshotPages` screenshots `page.locator('main')` by default; pass a `locator: (page) => ...` on the entry if a page needs something else (see `sidebar.spec.ts`).
2. Mock its data: add fixtures in `scripts/vrt/fixtures/<domain>.mjs`, wire them into `MOCK_UPSTREAMS` in `scripts/vrt/mock-upstreams.mjs`. Any image a fixture carries takes its URL from `vrtMockImageUrls` (`scripts/vrt/fixtures/images.mjs`) — generated SVG placeholders the mock server serves, never a real remote image. If the console warns an endpoint has no fixture, run `yarn vrt:pages:fixtures:single "<test name>"` to capture the real response into `.vrt-pages/missing-fixtures/` (gitignored) — review and redact it, then turn it into a fixture. Recording runs the page directly on the host (no Docker, no screenshots — `takeVrtScreenshot` no-ops in this mode) and is a separate, explicit step: an endpoint left unmocked keeps hitting the real backend on every normal run, so it still surfaces genuine backend-driven differences until you deliberately mock it.
3. If the page needs interaction before the screenshot (typing, clicking, waiting for a result), pass a `beforeScreenshot: async (page) => ...` on the entry. If that interaction is likely to repeat across pages (e.g. "search and wait for a result"), write it as a reusable function in `src/test/vrt-pages/interactions.ts` instead of inlining it (see `fillAndWaitForText`).
